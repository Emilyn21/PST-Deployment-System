...
$programStmt = $conn->prepare("SELECT id, program_name FROM tbl_program WHERE status = 'active' AND isDeleted = 0 AND withMajor = 1");
$programStmt->execute();
...
$stmt = $conn->prepare("SELECT tbl_major.*, tbl_program.program_name 
                        FROM tbl_major 
                        INNER JOIN tbl_program 
                        ON tbl_major.program_id = tbl_program.id 
                        WHERE tbl_major.isdeleted = 0 
                        ORDER BY tbl_major.created_at ASC");
$stmt->execute();
...
<head>
    ...
    <title>Manage Majors - Admin</title>
...
<body class="sb-nav-fixed">
        ...
        <div id="layoutSidenav_content">
            <main role="main">
                <div class="container-fluid px-4">
                    <h1 class="mt-5 h3" id="main-heading">Manage Majors</h1>
                    ...
                    <?php
                    function getMajorCount($conn, $condition) {
                        $stmt = $conn->prepare("SELECT COUNT(*) as total FROM tbl_major m WHERE $condition");
                        $stmt->execute();
                        $result = $stmt->get_result();
                    ...
                    $totalMajors = getMajorCount($conn, "isDeleted = 0");
                    $activeMajors = getMajorCount($conn, "status = 'active' AND isDeleted = 0");
                    $inactiveMajors = getMajorCount($conn, "status = 'inactive' AND isDeleted = 0");
                    $linkedMajors = getMajorCount($conn, "
                        EXISTS (SELECT 1 FROM tbl_program p WHERE m.program_id = p.id AND p.isDeleted = 0)
                        AND m.isDeleted = 0
                    ");
                    ?>
                    ...
                    <div class="row">
                        <div class="col-xl-3 col-md-6">
                            <div class="card bg-primary text-white mb-3" role="region" aria-label="Total Majors">
                                <div class="card-header d-flex align-items-center justify-content-between py-2">
                                    <div class="d-flex align-items-center">
                                        <i class="fas fa-bookmark me-2"></i>
                                        <span>Total Majors</span>
                                    </div>
                                    <h3 class="mb-0"><?= $totalMajors ?></h3>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-3 col-md-6">
                            <div class="card bg-success text-white mb-3" role="region" aria-label="Active Majors">
                                <div class="card-header d-flex align-items-center justify-content-between py-2">
                                    <div class="d-flex align-items-center">
                                        <i class="fas fa-check-circle me-2"></i>
                                        <span>Active Majors</span>
                                    </div>
                                    <h3 class="mb-0"><?= $activeMajors ?></h3>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-3 col-md-6">
                            <div class="card bg-warning text-white mb-3" role="region" aria-label="Inactive Majors">
                                <div class="card-header d-flex align-items-center justify-content-between py-2">
                                    <div class="d-flex align-items-center">
                                        <i class="fas fa-times-circle me-2"></i>
                                        <span>Inactive Majors</span>
                                    </div>
                                    <h3 class="mb-0"><?= $inactiveMajors ?></h3>
                                </div>
                            </div>
                        </div>
                        <div class="col-xl-3 col-md-6">
                            <div class="card bg-info text-white mb-3" role="region" aria-label="Linked Majors">
                                <div class="card-header d-flex align-items-center justify-content-between py-2">
                                    <div class="d-flex align-items-center">
                                        <i class="fas fa-link me-2"></i>
                                        <span>Majors Linked to Programs</span>
                                    </div>
                                    <h3 class="mb-0"><?= $linkedMajors ?></h3>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="card mb-4">
                        <div class="card-body">
                            <p class="mb-0" role="note">
                                Manage and monitor majors, ensuring alignment with academic programs.
                            </p>
                        </div>
                    </div>
                    <div class="card mb-4">
                        ...
                        <div class="card-body">
                            <div class="row mb-3">
                                <div class="col d-flex flex-wrap justify-content-end gap-2">
                                    ...
                                    <button onclick="exportTableToCSV('List of Majors.csv')" class="btn btn-secondary">CSV</button>
                                    <button onclick="exportTableToExcel('List of Majors.xlsx')" class="btn btn-secondary">Excel</button>
                                    <button onclick="printTable()" class="btn btn-secondary">Print</button>
                                </div>
                            </div>
                            <table id="datatablesSimple" class="table table-bordered" role="table" aria-label="Majors Table">
                                <thead role="rowgroup">
                                    <tr role="row">
                                ...
                                <tbody role="rowgroup">
                                    ...
                                    <tr role="row">
                                        <td role="cell"><?= htmlspecialchars($row['major_name'], ENT_QUOTES, 'UTF-8'); ?></td>
                                        <td role="cell"><?= htmlspecialchars($row['major_abbreviation'], ENT_QUOTES, 'UTF-8'); ?></td>
                                        <td role="cell"><?= htmlspecialchars($row['major_description'] ?? '', ENT_QUOTES, 'UTF-8'); ?></td>
                                        ...
                                        <td role="cell">
                                            <button class="btn btn-sm btn-warning btn-action" 
                                                onclick="openEditModal(
                                                    <?= $row['id']; ?>,
                                                    <?= htmlspecialchars(json_encode($row['major_name']), ENT_QUOTES, 'UTF-8'); ?>, 
                                                    <?= htmlspecialchars(json_encode($row['major_abbreviation']), ENT_QUOTES, 'UTF-8'); ?>, 
                                                    <?= htmlspecialchars(json_encode($row['major_description']), ENT_QUOTES, 'UTF-8'); ?>, 
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-warning">
                    <h5 class="modal-title" id="editModalLabel"><i class="fas fa-edit"></i> Edit Major</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form id="editForm" action="functions/update-major.php" method="POST">
                        <input type="hidden" id="editId" name="id" />
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label for="editName" class="form-label">Major Name</label>
                                <input type="text" class="form-control" id="editName" name="major_name" required />
                            </div>
                            <div class="col-md-6">
                                <label for="editAbbreviation" class="form-label">Major Abbreviation</label>
                                <input type="text" class="form-control" id="editAbbreviation" name="major_abbreviation" required />
                            </div>
                        </div>
                        <div class="mb-3">
                            <label for="editDescription" class="form-label">Major Description</label>
                            <textarea class="form-control" id="editDescription" name="major_description"></textarea>
                        </div>
                        <div class="row mb-3">
                            <div class="col-md-6">
                                <label for="editProgram" class="form-label">Program Name</label>
    ...
    <div class="modal fade" id="editSuccessModal" role="dialog" tabindex="-1" aria-labelledby="editSuccessModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    The major has been successfully updated.
                </div>
                <div class="modal-footer">
    ...
    <div class="modal fade" id="editErrorModal" role="dialog" tabindex="-1" aria-labelledby="editErrorModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    The major name and/or abbreviation already exists under the chosen program. Please enter different values.
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" data-bs-dismiss="modal">OK</button>
                </div>
    ...
    <div class="modal fade" id="deleteModal" role="dialog" tabindex="-1" aria-labelledby="deleteModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title" id="deleteModalLabel"><i class="fa fa-trash"></i> Delete Major</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
    ...
    <div class="modal fade" id="deleteSuccessModal" tabindex="-1" role="dialog" aria-labelledby="deleteSuccessModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    The major has been successfully deleted.
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" data-bs-dismiss="modal">OK</button>
                </div>
    ...
    <div class="modal fade" id="deleteErrorModal" tabindex="-1" role="dialog" aria-labelledby="deleteErrorModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    The major can't be deleted. There are registered pre-service teachers and related placements.
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-primary" data-bs-dismiss="modal">OK</button>
                </div>
    ...
    <script>
        window.addEventListener('DOMContentLoaded', event => {
            ...
            if (datatablesSimple) {
                const dataTable = new simpleDatatables.DataTable(datatablesSimple, {
                    labels: {
                        noRows: "No majors have been added yet."
                    },
            ...
            window.openEditModal = function(id, name, abbreviation, description, status, program_id) {
                ...
                let programSelect = document.getElementById('editProgram');
                ...
                document.getElementById('editForm').onsubmit = function(event) {
                    ...
                    fetch('functions/update-major.php', {
                        method: 'POST',
                        body: formData,
                        headers: {
            ...
            window.openDeleteModal = function(id, name) {
                ...
                document.getElementById('confirmDeleteButton').onclick = function() {
                    fetch('functions/delete-major.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                        body: JSON.stringify({ majorId: id })
                    })
                    .then(response => response.json())
                    .then(data => {
                        if (data.status === 'success') {
        ...
        function exportTableToCSV(filename) {
            ...
            const headers = ["Major Name", "Major Abbreviation", "Major Description", "Program Name", "Status"];
        ...
        function printTable() {
            ...
            win.document.write('<html><head><title>List of Majors</title>');
            win.document.write(`
                <style>
                    @page {
                        size: A4 portrait;
            ...
            `);
            win.document.write('<div class="header-container"><p class="title-text">List of Majors</p></div>');
            win.document.write('<table>');
            win.document.write('<thead><tr><th>Name</th><th>Abbreviation</th><th>Description</th><th>Program</th><th>Status</th><tr></thead>');
