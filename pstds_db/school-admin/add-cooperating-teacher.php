...
<head>
    ...
    <title>Add Cooperating Teacher - School Admin</title>
...
<body class="sb-nav-fixed">
        ...
        <div id="layoutSidenav_content">
            <main role="main">
                <div class="container-fluid px-4">
                    <h1 class="mt-5 h3" id="main-heading">Add Cooperating Teacher</h1>
                    <ol class="breadcrumb mb-4" role="navigation" aria-label="Breadcrumb">
                        <li class="breadcrumb-item"><a href="index.php">Dashboard</a></li>
                        <li class="breadcrumb-item active" aria-current="page">Add Cooperating Teacher</li>
                    </ol>
                    <section class="row" role="region" aria-labelledby="section-heading">
                        ...
                        <article class="col-md-12" role="article">
                            <div class="card mb-4" role="complementary">
                                <div class="card-body">
                                    <p class="mb-0" role="note">Fill in cooperating teacher details. Double check the email address.</p>
                                </div>
                            </div>
                            <!-- Form -->
                            <div class="card mb-4" role="complementary">
                                <div class="card-body">
                                    <form id="cooperatingTeacherForm" action="functions/process-add-cooperating-teacher.php" method="POST" class="main-content" role="form">
                                        ...
                                        <fieldset role="region" aria-labelledby="legend-academic-info" class="p-3 border rounded mt-4">
                                            ...
                                            <div class="row mb-3">
                                                <div class="col-md-4">
                                                    ...
                                                    <select id="subject-area" name="subject_area" class="form-select" role="combobox" required>
                                                        <option value="" disabled selected>Select subject area</option>
                                                        <?php
                                                            include '../connect.php';
                                                            $query = "SELECT id, subject_area_name FROM tbl_subject_area WHERE isDeleted = 0 AND status = 'active'"; // Get both id and name
                                                            $result = mysqli_query($conn, $query);
                                                            while ($row = mysqli_fetch_assoc($result)) {
                                                                echo '<option value="' . htmlspecialchars($row['id']) . '">' . htmlspecialchars($row['subject_area_name']) . '</option>';
                                                            }
                                                        ?>
                                                    </select>
                                                </div>
                                ...
                                <div class="card-footer">
                                    <button type="submit" form="cooperatingTeacherForm" class="btn btn-primary"><i class="fas fa-plus"></i> Add</button>
                                </div>
                            </div>
                        </article>
                    </section>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <a href="manage-cooperating-teacher.php" class="btn btn-primary" role="button">View List</a>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            const form = document.getElementById('cooperatingTeacherForm');
