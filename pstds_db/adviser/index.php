...
if ($activeSemesterId) {
    $stmt = $conn->prepare("
        ...
        LEFT JOIN
            tbl_adviser_assignment taa ON tpl.id = taa.placement_id
        WHERE 
            tu.isDeleted = 0
            AND taa.adviser_id = ?
    ...
    $stmt->bind_param("ii", $adviser_id, $activeSemesterId);
    $stmt->execute();
            ...
            <main role="main">
                <div class="container-fluid px-4">
                    ...
                    $stmt = $conn->prepare("SELECT COUNT(DISTINCT tpst.id) AS total_teachers
                                         ...
                                         WHERE tpl.isDeleted = 0 AND tu.isDeleted = 0
                                             AND taa.adviser_id = ? AND tpst.semester_id = ?");
                    $stmt->bind_param("ii", $adviser_id, $activeSemesterId);
                    $stmt->execute();
                    ...
                    $stmt = $conn->prepare("SELECT COUNT(DISTINCT tpst.id) AS total_pending_evaluations 
                                              ...
                                              LEFT JOIN tbl_adviser_assignment taa ON taa.placement_id = tpl.id
                                              WHERE tpl.isDeleted = 0 AND taa.adviser_id = ? 
                                                  AND (te.eportfolio_grade IS NULL OR te.internship_grade IS NULL OR te.final_demo_average IS NULL)
                                                  AND tpst.semester_id = ?");
                    ...
                    $stmt = $conn->prepare("SELECT COUNT(*) AS total_journal_entries 
                                                ...
                                                LEFT JOIN tbl_adviser_assignment taa ON tpl.id = taa.placement_id
                                                WHERE tpl.isDeleted = 0 AND taa.adviser_id = ?
                                                    AND tpst.semester_id = ?");
                    $stmt->bind_param("ii", $adviser_id, $activeSemesterId);
                    $stmt->execute();
